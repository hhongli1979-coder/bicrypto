...
export async function processWalletPnl() {
  ...
  try {
    ...
    logger.error("CRON", "Wallet PnL processing failed", error);
...
const handlePnl = async (user: any) => {
  ...
  try {
    ...
    logger.error("CRON", `Error handling PnL for user ${user.id}`, error);
    broadcastLog(
      "processWalletPnl",
...
export async function cleanupOldPnlRecords() {
  ...
  try {
    ...
    logger.error("CRON", "Cleanup of old PnL records failed", error);
    broadcastStatus(cronName, "failed");
    broadcastLog(
      cronName,
...
export async function processSpotPendingDeposits() {
  ...
  try {
    ...
    logger.error("CRON", "Processing pending spot deposits failed", error);
    broadcastStatus(cronName, "failed");
    broadcastLog(
      cronName,
...
export async function getPendingSpotTransactionsQuery(type: string) {
  try {
    ...
    logger.error("CRON", "Error getting pending spot transactions", error);
    throw error;
  }
}
  ...
  try {
    ...
    for (const transaction of transactions) {
      ...
      try {
        ...
        logger.error("CRON", `Error processing withdrawal ${transaction.id}`, error);
        broadcastLog(
          cronName,
    ...
    logger.error("CRON", "Processing pending withdrawals failed", error);
    broadcastStatus(cronName, "failed");
    broadcastLog(
      cronName,
