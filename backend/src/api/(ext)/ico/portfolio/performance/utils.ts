 ...
 */
export async function getAllocationByToken(
  userId: string,
  date: Date,
  ctx?: LogContext
) {
  try {
    ctx?.step?.("Fetching completed transactions for allocation calculation");
    ...
    ctx?.step?.("Calculating cumulative holdings per offering");
    ...
    ctx?.step?.("Computing market value per offering");
    ...
    ctx?.success?.(`Calculated allocation for ${allocationByToken.length} tokens`);

    return { allocationByToken, totalPortfolioValue: totalValue };
  } catch (error: any) {
    ctx?.fail?.(error.message || "Failed to calculate allocation by token");
    throw error;
  }
}
...
export async function getUserPortfolioHistory(
  ...
  try {
    ctx?.step?.("Fetching transaction history for portfolio calculation");
    ...
    ctx?.step?.("Processing transactions before start date");
    ...
    ctx?.step?.("Building daily portfolio history");
    ...
    ctx?.success?.(`Generated ${history.length} days of portfolio history`);

    return history;
  } catch (error: any) {
    ctx?.fail?.(error.message || "Failed to generate portfolio history");
    throw error;
  }
}
