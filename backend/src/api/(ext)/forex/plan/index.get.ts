...
export const metadata = { summary: "Get Forex Investment Plans",
  description:
...
export default async (data: Handler) => { const { user, query, ctx } = data as any;
  ...
  if (activeTab === "trending") { whereClause.trending = true;
  }
  ...
  if (sortBy === "profit") { orderClause.push(["profitPercentage", "DESC"]);
  } else if (sortBy === "minInvestment") { orderClause.push(["minAmount", "ASC"]);
  } else { orderClause.push([sequelizeInstance.literal("invested"), "DESC"]);
  }

  // Fetch the plans, compute "invested", and exclude unwanted fields.
  const plans = await models.forexPlan.findAll({ attributes: { // Exclude fields not intended for user display.
