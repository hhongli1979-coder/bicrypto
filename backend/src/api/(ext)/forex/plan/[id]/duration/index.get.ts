...
export const metadata = { summary: "Get durations for a specific Forex plan",
...
export default async (data: Handler) => { const { user, params, ctx } = data as any;
  ...
  try { // Check if plan exists
    const plan = await models.forexPlan.findOne({ where: { id, status: true },
    });
    ...
    const durations = await models.forexDuration.findAll({ include: [
