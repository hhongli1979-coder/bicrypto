...
export const metadata = { summary: "Retrieve specific Forex Investment Plan",
  description:
...
export default async (data: Handler) => { const { user, params, ctx } = data as any;
  if (!user?.id) { throw createError({ statusCode: 401, message: "Unauthorized" });
  }

  const { id } = params;

  try { const plan = await models.forexPlan.findOne({ where: { id, status: true },
      include: [
    ...
    if (!plan) { throw createError({ statusCode: 404, message: "Forex Plan not found" });
    }

    // Count distinct investors for this plan.
    const totalInvestors = await models.forexInvestment.count({ where: { planId: id },
      distinct: true,
      col: "userId",
    });

    // Calculate the total invested amount by summing the "amount" field for all investments in this plan.
    const invested = await models.forexInvestment.sum("amount", { where: { planId: id },
    });
  ...
  ctx?.success("Request completed successfully");
    ...
    return planJson;
  } catch (error) { console.error("Error fetching forex plan:", error);
