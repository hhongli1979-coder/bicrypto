...
export async function processAiInvestments() {
  ...
  try {
    ...
    logger.error("AI_INVESTMENT_PROCESS", `AI investments processing failed: ${error.message}`, error);
...
export async function getActiveInvestments() {
  try {
    ...
    logger.error("AI_INVESTMENT_PROCESS", "Failed to get active investments", error);
    throw error;
  }
}
...
export async function processAiInvestment(investment: any) {
  ...
  try {
    ...
    logger.error("AI_INVESTMENT_PROCESS", `General error processing AI investment ${investment.id}: ${error.message}`, error);
    broadcastLog(
      cronName,
...
async function handleAiInvestmentUpdate(
  ...
  try {
    ...
    logger.error("AI_INVESTMENT_UPDATE", `Error updating AI investment: ${error.message}`, error);
    return null;
  }
  return updatedInvestment;
}
...
async function postProcessAiInvestment(
  ...
  try {
    ...
    logger.error("AI_INVESTMENT_POST_PROCESS", `Error in postProcessAiInvestment: ${error.message}`, error);
  }
}
