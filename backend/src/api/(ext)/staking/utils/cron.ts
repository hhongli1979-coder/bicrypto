...
import { sendStakingRewardEmail } from "@b/utils/emails";
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.error(
            "STAKING",
            `Failed to send email for position ${positionWithLock.id} (User ${positionWithLock.user.id})`,
            emailErr
          );
          broadcastLog(
            cronName,
        ...
        try {
          ...
          logger.error(
            "STAKING",
            `Failed to create notification for position ${positionWithLock.id} (User ${positionWithLock.user.id})`,
            notifErr
          );
          broadcastLog(
            cronName,
        ...
        try {
          ...
          logger.error(
            "STAKING",
            `Failed to process additional rewards for position ${positionWithLock.id} (User ${positionWithLock.user.id})`,
            rewardErr
          );
          broadcastLog(
            cronName,
        ...
        logger.error(
          "STAKING",
          `Transaction failed for position ${pos.id} (User ${pos.user?.id})`,
          txnError
        );
      ...
      logger.error("STAKING", `Failed to process position ${pos.id}`, error);
...
async function getSettingsWithFallback() {
  try {
    ...
    logger.warn("STAKING", "Cache retrieval failed, falling back to database settings");
...
export async function processStakingPositions() {
  ...
  try {
    ...
    const results = await processWithConcurrency(
      ...
      async (pos: any) => {
        try {
          ...
          logger.error(
            "STAKING",
            `Error processing position ${pos.id} (User ${pos.user?.id})`,
            posError
          );
          return {
            success: false,
    ...
    if (failureCount > 0) {
      ...
      logger.error("STAKING", `Failed to process ${failureCount} positions: ${failedPositionIds.join(", ")}`);
    ...
    logger.error("STAKING", "Processing of staking positions failed", error);
