...
export async function listDirectReferrals(user, ctx?: LogContext) {
  try {
    ctx?.step?.("Fetching direct referrals");
    ...
    ctx?.step?.("Processing referrals data");
    ...
    ctx?.success?.("Direct referrals fetched successfully");
    return result;
  } catch (error: any) {
...
export async function listUnilevelReferrals(user, mlmSettings, ctx?: LogContext) {
  try {
    ctx?.step?.("Fetching unilevel referrals");
    ...
    async function buildDownlines(referrals, level) {
      ...
      ctx?.step?.(`Building downlines for level ${level}`);
      const downlines: any = [];
    ...
    ctx?.step?.("Building downline hierarchy");
    rootUser.downlines = await buildDownlines(directReferrals, 2);

    ctx?.success?.("Unilevel referrals fetched successfully");
    return rootUser;
  } catch (error: any) {
    ctx?.fail?.(error.message);
    throw error;
...
export async function listBinaryReferrals(user, mlmSettings, ctx?: LogContext) {
  try {
    ctx?.step?.("Fetching binary referrals");
    const referrerId = user.id;
    ...
    if (!rootNode) {
      ctx?.step?.("No root node found, returning empty structure");
      return {
        id: user.id,
        firstName: user.firstName,
        lastName: user.lastName,
    ...
    async function fetchBinaryDownlines(nodeIds, level = 2) {
      ...
      ctx?.step?.(`Fetching binary downlines for level ${level}`);
    ...
    ctx?.step?.("Building binary tree structure");
    ...
    ctx?.success?.("Binary referrals fetched successfully");
    return result;
  } catch (error: any) {
    ctx?.fail?.(error.message);
    throw error;
