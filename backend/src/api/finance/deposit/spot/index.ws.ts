...
import { updateSpotWalletBalance } from "@b/utils/spot";
import {
...
const sendMessage = (payload, data) => {
  try {
    ...
    logger.error("SPOT_DEPOSIT", `Failed to send message: ${error}`);
  }
};
...
export function startSpotVerificationSchedule(
  ...
  const interval = setInterval(async () => {
    try {
      ...
      logger.error("SPOT_DEPOSIT", `Error verifying transaction: ${error.message}`);
...
export async function verifyTransaction(
  ...
  logger.debug("SPOT_DEPOSIT", `Starting verification for transaction ${trx} (User: ${userId})`);
  
  try {
    const transaction = await getTransactionQuery(userId, trx);

    if (!transaction) {
      logger.error("SPOT_DEPOSIT", `Transaction not found for trx: ${trx}, userId: ${userId}`);
      throw new Error("Transaction not found");
    }

    const wallet = await models.wallet.findByPk(transaction.walletId);
    if (!wallet) {
      logger.error("SPOT_DEPOSIT", `Wallet not found for transaction ${transaction.id}`);
      throw new Error("Wallet not found");
    }

    logger.debug("SPOT_DEPOSIT", `Processing transaction ${transaction.id} for currency ${wallet.currency}`);

    // Parse metadata and get chain information
    const { metadata, xtChain } = parseMetadataAndMapChainToXt(transaction.metadata);
    logger.debug("SPOT_DEPOSIT", `Chain: ${metadata.chain}, XT mapped chain: ${xtChain}`);

    if (transaction.status === "COMPLETED") {
      logger.debug("SPOT_DEPOSIT", `Transaction ${transaction.id} already completed`);
      sendMessage(payload, {
        status: 201,
    ...
    // Initialize exchange
    logger.debug("SPOT_DEPOSIT", "Initializing exchange connection...");
    const exchange = await ExchangeManager.startExchange();
    if (!exchange) {
      logger.error("SPOT_DEPOSIT", "Exchange instance not available - this could indicate missing exchange configuration");
      ...
      if (!activeProvider) {
        logger.error("SPOT_DEPOSIT", "No active exchange provider found in database");
        sendMessage(payload, {
          status: 500,
      ...
      logger.error("SPOT_DEPOSIT", `Exchange provider '${activeProvider.name}' is configured but connection failed`);
      sendMessage(payload, {
        status: 500,
    ...
    if (!provider) {
      logger.error("SPOT_DEPOSIT", "Provider name not available");
      sendMessage(payload, {
        status: 500,
    ...
    logger.debug("SPOT_DEPOSIT", `Using exchange provider: ${provider}`);

    try {
      const credentialsTest = await ExchangeManager.testExchangeCredentials(provider);
      if (!credentialsTest.status) {
        logger.error("SPOT_DEPOSIT", `Exchange credentials test failed: ${credentialsTest.message}`);
        sendMessage(payload, {
          status: 500,
      ...
      logger.debug("SPOT_DEPOSIT", "Exchange credentials verified successfully");
    } catch (error) {
      logger.error("SPOT_DEPOSIT", `Error testing exchange credentials: ${error.message}`);
      // Continue with verification attempt but log the error
    }

    logger.debug("SPOT_DEPOSIT", `Fetching deposits for currency ${wallet.currency}...`);
    ...
    try {
      if (exchange.has["fetchDeposits"]) {
        ...
        if (xtChain && provider === "xt") {
          ...
          logger.debug("SPOT_DEPOSIT", `Using XT chain parameter: ${xtChain}`);
        }
        // For KuCoin, we need to handle TRC20 properly
          ...
          logger.debug("SPOT_DEPOSIT", `Using KuCoin chain parameter: ${kucoinChain} (original: ${metadata.chain})`);
        }

        // For KuCoin debugging - let's try multiple approaches
        if (provider === "kucoin") {
          logger.debug("SPOT_DEPOSIT", "KuCoin Debug - Testing different parameter combinations...");

          // Try 1: With chain parameter
          logger.debug("SPOT_DEPOSIT", "KuCoin Try #1: With chain='TRX' parameter");
          ...
          logger.debug("SPOT_DEPOSIT", `KuCoin Try #1 Result: ${deposits.length} deposits`);

          // If no results with chain, try without chain parameter
          if (deposits.length === 0) {
            logger.debug("SPOT_DEPOSIT", "KuCoin Try #2: Without chain parameter");
            const depositsNoChain = await exchange.fetchDeposits(wallet.currency);
            logger.debug("SPOT_DEPOSIT", `KuCoin Try #2 Result: ${depositsNoChain.length} deposits`);

            if (depositsNoChain.length > 0) {
              deposits = depositsNoChain;
              logger.debug("SPOT_DEPOSIT", "KuCoin: Using results from Try #2 (no chain parameter)");
            }
          }

          // If still no results, try fetching ALL deposits (no currency filter)
          if (deposits.length === 0) {
            logger.debug("SPOT_DEPOSIT", "KuCoin Try #3: Fetching ALL deposits (no currency filter)");
            try {
              const allDeposits = await exchange.fetchDeposits();
              logger.debug("SPOT_DEPOSIT", `KuCoin Try #3 Result: ${allDeposits.length} total deposits`);

              if (allDeposits.length > 0) {
                // Filter for TRX deposits manually
                const trxDeposits = allDeposits.filter(d => d.currency === 'TRX');
                logger.debug("SPOT_DEPOSIT", `KuCoin Try #3: Found ${trxDeposits.length} TRX deposits out of ${allDeposits.length} total`);

                if (trxDeposits.length > 0) {
                  deposits = trxDeposits;
                  logger.debug("SPOT_DEPOSIT", "KuCoin: Using filtered TRX deposits");
                }
              }
            } catch (allDepositsError) {
              logger.error("SPOT_DEPOSIT", `KuCoin Try #3 Error: ${allDepositsError.message}`);
            }
          }
        } else {
        ...
        logger.debug("SPOT_DEPOSIT", `Found ${deposits.length} deposits using fetchDeposits`);

        // Log first few deposits for debugging
        if (deposits.length > 0) {
          logger.debug("SPOT_DEPOSIT", `Sample: ${deposits.slice(0, 3).map(d => `${d.currency}:${d.amount}`).join(', ')}`);
        } else {
          logger.debug("SPOT_DEPOSIT", "No deposits found - deposit may not have arrived yet");
        }
      } else if (exchange.has["fetchTransactions"]) {
        deposits = await exchange.fetchTransactions();
        logger.debug("SPOT_DEPOSIT", `Found ${deposits.length} transactions using fetchTransactions`);
      } else {
        logger.error("SPOT_DEPOSIT", `Exchange ${provider} does not support fetchDeposits or fetchTransactions`);
        sendMessage(payload, {
          status: 500,
      ...
      logger.error("SPOT_DEPOSIT", `Error fetching deposits or transactions: ${error.message}`);
    ...
    logger.debug("SPOT_DEPOSIT", `Searching for transaction ${trx} in ${deposits.length} deposits...`);
    let deposit;
    if (provider === "binance") {
      deposit = deposits.find((d) => {
        ...
        if (matches) {
          logger.debug("SPOT_DEPOSIT", `Found matching Binance deposit: ${d.txid}`);
        }
        return matches;
      });
    } else {
      ...
      deposit = deposits.find((d) => {
        ...
        if (matches) {
          logger.debug("SPOT_DEPOSIT", `Found matching deposit: ${d.txid}`);
        }
        return matches;
      });
    }
    ...
    if (!deposit) {
      logger.debug("SPOT_DEPOSIT", `Transaction ${trx} not found in exchange deposits yet`);
      return;
    }

    logger.debug("SPOT_DEPOSIT", `Found deposit with status: ${deposit.status}, amount: ${deposit.amount}`);

    if (deposit.status !== "ok") {
      logger.debug("SPOT_DEPOSIT", `Deposit status is not 'ok': ${deposit.status}`);
      return;
    }
    ...
    logger.debug("SPOT_DEPOSIT", `Processing deposit: amount=${amount}, fee=${fee}, currency=${deposit.currency || wallet.currency}`);

    if (
      ["kucoin", "binance", "okx", "xt"].includes(provider) &&
      wallet.currency !== deposit.currency
    ) {
      logger.error("SPOT_DEPOSIT", `Currency mismatch: wallet=${wallet.currency}, deposit=${deposit.currency}`);
      sendMessage(payload, {
        status: 400,
    ...
    if (
      ...
      if (
        ...
        logger.warn("SPOT_DEPOSIT", `Deposit expired: timeDiff=${timeDiff.toFixed(1)} minutes`);
        sendMessage(payload, {
          status: 400,
    ...
    logger.debug("SPOT_DEPOSIT", `Updating transaction ${transaction.id} to COMPLETED`);
    ...
    logger.debug("SPOT_DEPOSIT", `Updating wallet balance for user ${userId}`);
    ...
    if (!updatedWallet) {
      logger.error("SPOT_DEPOSIT", "Failed to update wallet balance");
      sendMessage(payload, {
        status: 500,
    ...
    if (provider === "kucoin") {
      try {
        ...
        logger.debug("SPOT_DEPOSIT", "Completed KuCoin transfer from main to trade account");
      } catch (error) {
        logger.error("SPOT_DEPOSIT", `Transfer failed: ${error.message}`);
      }
    }
    ...
    try {
      ...
      logger.debug("SPOT_DEPOSIT", "Sent confirmation email and notification");
    } catch (error) {
      logger.error("SPOT_DEPOSIT", `Deposit confirmation email failed: ${error.message}`);
    }

    try {
      await processRewards(userData.id, amount, "WELCOME_BONUS", wallet.currency);
      logger.debug("SPOT_DEPOSIT", "Processed welcome bonus rewards");
    } catch (error) {
      logger.error("SPOT_DEPOSIT", `Error processing rewards: ${error.message}`);
    }

    logger.success("SPOT_DEPOSIT", `Successfully completed deposit ${trx} for user ${userId}`);
    sendMessage(payload, {
      status: 200,
    ...
    logger.error("SPOT_DEPOSIT", `Error in verifyTransaction: ${error.message}`);
