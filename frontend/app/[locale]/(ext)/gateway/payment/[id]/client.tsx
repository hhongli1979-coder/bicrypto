...
const STATUS_CONFIG: Record<string, { color: string; bgColor: string; icon: any; label: string }> = {
  ...
  EXPIRED: { color: "text-gray-500", bgColor: "bg-gray-400/10 border-gray-400/20", icon: Timer, label: "Expired" },
  REFUNDED: { color: "text-purple-600", bgColor: "bg-purple-500/10 border-purple-500/20", icon: RefreshCcw, label: "Refunded" },
  PARTIALLY_REFUNDED: { color: "text-purple-500", bgColor: "bg-purple-400/10 border-purple-400/20", icon: RefreshCcw, label: "Partially Refunded" },
};
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <code className="text-sm text-muted-foreground font-mono">{payment.id}</code>
              <Button
                variant="ghost"
                size="icon"
                className="h-6 w-6"
            ...
            <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-xl bg-background/60 backdrop-blur-sm border">
                  ...
                  <p className="text-lg font-semibold text-orange-600">-{formatCurrency(payment.feeAmount, payment.currency)}</p>
                </div>
                <div className="p-4 rounded-xl bg-background/60 backdrop-blur-sm border border-green-500/20">
                  ...
                  <p className="text-lg font-semibold text-green-600">{formatCurrency(payment.netAmount, payment.currency)}</p>
                </div>
              </div>
            </div>
          </Card>
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm text-muted-foreground">
                            ≈ {formatCurrency(allocation.equivalentInPaymentCurrency, payment.currency)}
                          </p>
                        </div>
                      </div>
                    );
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          Qty: {item.quantity} × {formatCurrency(item.unitPrice, payment.currency)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-bold">
                          {formatCurrency(item.quantity * item.unitPrice, payment.currency)}
                        </p>
                      </div>
                    </div>
                  ))}
